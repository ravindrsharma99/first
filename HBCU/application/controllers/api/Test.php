...
if(empty($user_id) OR empty($card_id)  OR empty($hbcu) OR empty($amount))
{
  $response= $this->User_model->errorResponse("Required fields are missing");
}else
{
    ...
    }else
    {
         ...
         function forgotpassword_post() {
          ...
          if ($result == 0)
          {
            ...
            $this->email->from('mymail.com', 'HBCU');
            $this->email->to($email);
...
//hb_reoccurringDonations
  ...
  public function getDonationstatusDetail_post(){
    ...
    public function CronspareChange_get(){
        $ReoccurringPayments  = $this->User_model->select_data('*','hb_users',array('isActivesparechange'=>1,'user_type'=>0));
        $date=date('Y-m-d');
        $dateTime=date('Y-m-d H:i:s');
        $count=array();
        $sd=array();
        ...
        // print_r($ReoccurringPayments);die;
        /*amount should be grater then 5 */
        $abc=array();
        foreach ($ReoccurringPayments as $key => $value) {
            if ($value->sum>=5) {
                $abc[]=$value;
                ...
                if (empty($UserHBCU11)) {
                    ...
                    foreach ($UserHBCU  as $key => $valueHBCU)
                    {
                        if($valueHBCU->donation_percent>0){
                            ...
                            echo "fdjis;d";
                            // Since the amount charged by stripe is in cents for singapore doller, so here amount = 1 is being converted into 100 cents since actual payment will be 1 doller from front end |here| 1 doller = 100 cents:
                            $pay =  \Stripe\Charge::create(array(
                            "amount"   => $stripeAmount,
                            "currency" => "USD",
                            "customer" => $customer_id
...
public function unapprovedCron_GET(){
  ...
  public function approvedCron_get(){
    ...
    public function aad_get(){
      $this->load->library('email');

      $this->email->from('mymail.com', 'Ben');
    ...
    public function abcd_get(){
                $checklogin  = $this->db->query("SELECT * from hb_users where signup_level=5 and access_token_status=1")->result();
                foreach ($checklogin as $key => $value) {
                  if ($value->pushNotifications==0) {
                    $checklogin=$this->User_model->select_data('*','hb_login',array('user_id'=>$value->id,'status'=>1));
                    print_r($checklogin);

                    foreach ($checklogin as $key => $value) {
                      $pushData['message'] = "There is an update of app, please upgrade your app for the upgraded features.";
                      $pushData['action'] = "referral";
                      $pushData['token'] = $value->token_id;
                      if($value->login_via == 1){
                        $this->User_model->iosPush($pushData);
          ...
          public function stripeplan_post(){
              // \Stripe::setApiKey("sk_test_AV45bRuYmLu0nLFvLggzTQjF");

           // $abc= \Stripe\Subscription::create(array(
           //  "customer" => "cus_CWhLCDoMr2c6mO",
           //  "items" => array(
           //  array(
           //  "plan" => "gold",
           //  ),
